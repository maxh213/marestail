...
def test_skips_unchanged_scope(tmp_path: Path) -> None:
    ...
    assert shape(untimed(er_lint.run_gate(ctx), er_lint.GATE)) == ("er.lint", True, "skipped: no changed erlang files", [])


def test_skips_without_files(tmp_path: Path) -> None:
    result = untimed(er_lint.run_gate(project(tmp_path, ())), er_lint.GATE)
...
def test_clean(tmp_path: Path, fake_run: Callable[..., FakeRun]) -> None:
    ...
    result = checked(er_lint.run_gate(project(tmp_path, scope_changed=True, changed={"src/a.hrl"})), er_lint.GATE)
...
def test_successful_erlc_keeps_its_warnings_out_of_the_findings(tmp_path: Path, fake_run: Callable[..., FakeRun]) -> None:
    fake_run(erlang, [(0, f"{tmp_path / 'src' / 'a.erl'}:3:5: Warning: variable 'X' is unused\n"), (0, "")])
    result = checked(er_lint.run_gate(project(tmp_path)), er_lint.GATE)
    assert shape(result) == ("er.lint", True, "erlc strong warnings clean", [])


def test_only_tests(tmp_path: Path, fake_run: Callable[..., FakeRun]) -> None:
    fake = fake_run(erlang, [(0, "")])
    checked(er_lint.run_gate(project(tmp_path, ("tests/t.erl",))), er_lint.GATE)
    assert [call[1] for call in fake.calls] == ["-DTEST"]


def test_only_sources(tmp_path: Path, fake_run: Callable[..., FakeRun]) -> None:
    fake = fake_run(erlang, [(0, "")])
    checked(er_lint.run_gate(project(tmp_path, ("src/a.erl",))), er_lint.GATE)
...
def test_hint_stops(tmp_path: Path, fake_run: Callable[..., FakeRun]) -> None:
    ...
    assert shape(checked(er_lint.run_gate(project(tmp_path)), er_lint.GATE)) == ("er.lint", False, HINT, [HINT])
