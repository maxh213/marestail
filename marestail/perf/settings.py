...
def threshold_percent(config: Config) -> float:
    return float(config.get(SECTION, "threshold_percent", 10))


def min_runs(config: Config) -> int:
    return int(config.get(SECTION, "min_runs", 10))


def values_per_sample(config: Config) -> int:
    return int(config.get(SECTION, "values_per_sample", 200))


def p95_min_values(config: Config) -> int:
    return int(config.get(SECTION, "p95_min_values", 200))


def bootstrap_rounds(config: Config) -> int:
    return int(config.get(SECTION, "bootstrap", 500))


def control(config: Config) -> bool:
    return config.get(SECTION, "control", False) is True


def min_change(config: Config) -> tuple[tuple[str, float], ...]:
    configured = config.get(SECTION, "min_change", DEFAULT_MIN_CHANGE)
...
def sample_timeout(config: Config) -> int:
    return int(config.get(SECTION, "sample_timeout", 600))


def db(config: Config) -> dict[str, Any]:
    section = config.get(SECTION, "db", {})
...
def effective_rows(config: Config) -> tuple[int, str]:
    ...
    if "rows" in db(config):
        return parse_rows(db(config)["rows"]), TOML
...
def parse_rows(value: object) -> int:
    if whole_int(value):
        return value
    if digit_string(value):
        return int(value)
    raise ValueError(f"[perf.db] rows must be a whole number ≥ 0, got {value}")


def whole_int(value: object) -> TypeGuard[int]:
    return isinstance(value, int) and not isinstance(value, bool) and value >= 0


def digit_string(value: object) -> TypeGuard[str]:
