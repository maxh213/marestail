...
5. In `tools/test-timeline.py`, double `Run.gates` so hardener's pre-loop `gate_for` returns `[Result(gate="sonar", ok=False, summary="fail", seconds=41.0)]`, then run:
   ...
   Expected: `gate_for` / `state.gates` invoked once before attempts (not per attempt); every reused attempt step has `gate` containing `{"name": "sonar", "seconds": 41.0, "ok": false}`; `agent` present.
...
7. With `MARESTAIL_LIMIT_WAIT_SECONDS=0`, `--model dandelion/route`, stub dandelion plan `1 none` then `0 claude-opus-5 high claude`, stub claude succeeds.
   Expected: step `waits` includes `{"reason": "dandelion-unrouted", "seconds": 0}`; stdout still has `dandelion/route:`.
8. Cause two attempts of the same role.
   Expected: `timeline.json` has two steps; `timeline.md` has two `##` sections; ids match.
9. From a temp repo that already has `.marestail/runs/t/timeline.md`, run `tools/overnight.sh tasks/t.md` (stub agent, `STOP_AT` short).
...
10. `grep -E 'timeline\.(md|json)' README.md` near the Overnight `.marestail/runs/overnight-<stamp>.md` mention.
    Expected: both names present; no new false claim of `.marestail/runs/<task>/pipeline.log`.
11. Confirm freeze lists the timeline artefacts (e.g. `grep -E 'timeline\.(md|json)' marestail/freeze.py`).
